...
public class Transaction {
	 ...
	 * 
	 * @param context
	 *            is the context of the activity or service
	 */
	 ...
	 *            is the context of the activity or service
	 * @param settings
	 *            is the settings object to process send requests through
	 */
	...
	/**
	 * Called to send a new message depending on settings and provided Message object If you want to send message as mms, call this from the UI thread
	 * 
	 * @param message
	 *            is the message that you want to send
	 * @param threadId
	 *            is the thread id of who to send the message to (can also be set to Transaction.NO_THREAD_ID)
	 * @throws IOException
	 * @throws SocketException
	 */
	public void sendNewMessage(Message message, long threadId) throws SocketException, IOException {
		saveMessage = message.getSave();
		if (checkMMS(message)) {
	...
	private void sendSmsMessage(String text, String[] addresses, long threadId) {
		...
		for (String address : addresses) {
			...
			if (saveMessage) {
				...
				smsOutboxUri = Sms.Outbox.addMessage(context.getContentResolver(), address, settings.getStripUnicode() ? StripAccents.stripAccents(text) : text, "", cal.getTimeInMillis(), settings.getDeliveryReports(), threadId);
			}
