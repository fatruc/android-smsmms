...
public class Settings {
	 ...
	 * @param proxy
	 *            is the proxy address in the apn to send MMS through
	 * @param port
	 ...
	 * @param wifiMmsFix
	 *            is a boolean to toggle on and off wifi when sending MMS (MMS will not work currently when WiFi is enabled)
	 * @param deliveryReports
	 *            is a boolean to retrieve delivery reports from SMS messages
	 * @param split
	 *            is a boolean to manually split messages (shouldn't be necessary, but some carriers do not split on their own)
	 * @param splitCounter
	 *            adds a split counter to the front of all split messages
	 * @param stripUnicode
	 *            replaces many unicode characters with their gsm compatible equivalent to allow for sending 160 characters instead of 70
	 * @param signature
	 *            a signature to attach at the end of each message
	 * @param sendLongAsMms
	 *            if a message is too long to be multiple SMS, convert it to a single MMS
	 * @param sendLongAsMmsAfter
	 *            is an int of how many pages long an SMS must be before it is split
	 * @param account
	 *            is the google account to send Google Voice messages through
	 * @param rnrSe
	 *            is the token to use to send Google Voice messages (nullify if you don't know what this is)
	 ...
	 *            a signature to attach at the end of each message
	 * @param preText
	 *            text to be inserted before a message
	 * @param sendLongAsMms
	 *            if a message is too long to be multiple SMS, convert it to a single MMS
	 * @param sendLongAsMmsAfter
	 *            is an int of how many pages long an SMS must be before it is split
	 ...
	 * 
	 * @param preText
	 *            text to be attached to the beginning of each message
	 */
